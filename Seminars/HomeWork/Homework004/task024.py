...
for i in range(0, quantity):
    bushes.append(random.randint(0, 50)) #Количество ягод генерируем случайно от 0 до 50
print(bushes)

setmax = bushes[0] + bushes[-1] + bushes[1] # считаем что максимум ягод собрали с первого раза

for i in range(1, len(bushes)):
    if i == len(bushes) - 1:
        set = bushes[i] + bushes[i - 1] + bushes[0]
        if set > setmax:
            setmax = set
    else:
        set = bushes[i] + bushes[i - 1] + bushes[i + 1]
        if set > setmax:
            setmax = set

print(f'максимальное количество ягод: {setmax}')
